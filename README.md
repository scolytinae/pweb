...
1. Мне нравится [timeweb](https://timeweb.cloud/), но можно и любой другой
    (правда может быть сложнее настроить доступ на сервер по ключу).
    Создаем там облачный сервер с нужной локацией и 
    ...
    доступ по SSH с.clowd помощью пароля (они после п.0 должны лежать в `~/.ssh/` 
    под именем `*.pub`)
...
2. В папке `playbooks` создаем файл `inventory.yml` такой же как,
    ...
    **Внимательно!** Все символы `$` надо заменить на `$$`
...
3. После этого идем из папки `playbooks` запускаем команду
    ...
    Если все прошло хорошо, то по в браузере теперь можно по адресу `http://<your-server-ip>:51821/`
